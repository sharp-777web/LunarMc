...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const staff: { name: string; role: string; username: string }[] = [];
    ...
    for (const m of allMembers) {
      ...
      if (bestRank) {
        ...
        staff.push({ name, role: bestRank, username: (m.user.username || "").toLowerCase() });
      }
    }
    ...
    const publicStaff = staff.map(({ name, role }) => ({ name, role }));
